	...
	if ((ok = read_response(psb, SMTP_CMD_TIMEOUT))) {
		if ((ok = check_response(psb, TRUE))) {
			...
			digest_string = g_compute_hmac_for_data(G_CHECKSUM_MD5,
					psb->auth_secret, strlen(psb->auth_secret), chall, chall_size);
			...
			reply64 = g_base64_encode(reply, strlen(reply));
...
smtp_out_auth_login(smtp_base *psb)
{
	...
	if ((ok = read_response(psb, SMTP_CMD_TIMEOUT))) {
		if ((ok = check_response(psb, TRUE))) {
			...
			reply64 = g_base64_encode(psb->auth_login, strlen(psb->auth_login));
			fprintf(psb->out, "%s\r\n", reply64);
			fflush(psb->out);
			DEBUG(6) debugf("C: %s\n", reply64);
			g_free(reply64);
			if ((ok = read_response(psb, SMTP_CMD_TIMEOUT))) {
				if ((ok = check_response(psb, TRUE))) {
					...
					reply64 = g_base64_encode(psb->auth_secret, strlen(psb->auth_secret));
					fprintf(psb->out, "%s\r\n", reply64);
					fflush(psb->out);
					DEBUG(6) debugf("C: %s\n", reply64);
					g_free(reply64);
