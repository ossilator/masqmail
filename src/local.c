...
gboolean append_file(message *msg, GList *hdr_list, gchar *user)
{
  ...
  if((pw = getpwnam(user))){
	...
	logwrite(LOG_ALERT,
		 ...
		 strerror(errno));
	logwrite(LOG_ALERT,
		 "uid=%d, gid=%d, euid=%d, egid=%d, want = %d, %d\n",
	  ...
	  if(i == 3){
	    ...
	    */
	    ...
	    if((out = fopen(filename, "w"))){
	      gchar *newname =
		g_strdup_printf("%s/new/%s.%s", path, msg->uid, conf.host_name);
		...
		ok = FALSE;
		logwrite(LOG_ALERT, "moving %s to %s failed: %s",
...
pipe_out(message *msg, GList *hdr_list, address *rcpt, gchar *cmd, guint flags)
{
  ...
  /* set environment */
  {
    ...
    envp[i++] = g_strdup_printf("RETURN_PATH=%s@%s",
