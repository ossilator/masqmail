...
map_address_header(header *hdr, GList *table, GList *table2)
{
	...
	for (;;) {
		...
		if (rewr) {
			...
			newer_hdr = g_strdup_printf("%s%.*s", new_hdr ? new_hdr : "",
			                            (int)(addr_end - op), op);
		}
		g_free(new_hdr);
		new_hdr = newer_hdr;
	...
	header *out = g_malloc(sizeof(header));
	out->id = hdr->id;
	out->header = new_hdr;
	out->value = new_hdr + (hdr->value - hdr->header);
	return out;
}
...
rewrite_headers(msg_out *msgout, const connect_route *route)
{
	...
	foreach (msgout->hdr_list, hdr_node) {
		...
		new_hdr = map_address_header(hdr, table, table2);
		if (!new_hdr) {
			continue;
		}
	  replace:
