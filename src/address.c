...
destroy_recipient(recipient *addr)
{
	if (!--addr->ref_count) {
		...
		g_list_free_full(addr->children, (GDestroyNotify) destroy_recipient);
		g_free(addr);
	}
}
...
address*
copy_modify_address(const address *orig, const gchar *l_part, const gchar *dom)
{
	if (!orig) {
		return NULL;
	}
	return create_address_raw(l_part ? l_part : orig->local_part,
	                          dom ? dom : orig->domain);
}
